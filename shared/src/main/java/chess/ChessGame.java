...
//     Helper functions
    private ChessPosition getKingPosition(TeamColor team, ChessBoard board) {
        for (int i = 1; i <= 8; i++) {
            for (int j = 1; j <= 8; j++) {
    ...
    private boolean isKingAttacked(TeamColor teamColor, ChessBoard board) {
    ...
    public Collection<ChessMove> validMoves(ChessPosition startPosition) {
        ...
        for (ChessMove move : potentialMoves) {
            ...
            if (!isKingAttacked(startPiece.getTeamColor(), tempBoard)) {
                validMoves.add(move);
            }
        }
        return validMoves;
        ...
        if (startPiece.getPieceType() == ChessPiece.PieceType.PAWN) {
            ...
            if (end.getRow() == promotionRow) {
                if (move.getPromotionPiece() == null) {
                    throw new InvalidMoveException("Promotion piece required.");
                }

                board.movePiece(move);
                board.addPiece(end, new ChessPiece(startPiece.getTeamColor(), move.getPromotionPiece()));
            } else {
                board.movePiece(move);
            }
        } else {
    ...
    public boolean isInCheck(TeamColor teamColor) {
        return isKingAttacked(teamColor, board);
    }
